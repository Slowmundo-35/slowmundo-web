...
export const trip = defineType({
  ...
  fields: [
    ...
    defineField({
      name: "countries",
      title: "Autres pays de l’itinéraire",
      type: "array",
      group: "essentials",
      description: "Ajoutez chaque autre pays traversé (ex. Suisse, Italie). Le voyage apparaîtra dans chacun de ces pays. Le pays principal conserve l’adresse du voyage.",
      of: [{ type: "string", validation: (r) => r.required().max(60) }],
      options: { layout: "tags" },
      validation: (r) => r.unique().max(50),
    }),
    defineField({
      name: "continent",
      title: "Continent",
